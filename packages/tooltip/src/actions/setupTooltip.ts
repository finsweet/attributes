...
import { getAttribute, rotateArrow, type TooltipInstance } from '../utils';
...
export const setupTooltip = (
  ...
  const virtual = getAttribute(target, 'virtual') === 'true';
  const animation = getAttribute(target, 'animation') as keyof typeof animations;
  const animationEasing = (getAttribute(target, 'easing') as Easings[number]) || undefined;
  const animationDuration = Number(getAttribute(target, 'duration')) || undefined;
  ...
  const placement = (getAttribute(target, 'placement') || 'top') as Placement;
  const arrowPadding = getAttribute(target, 'padding') || '0';
  const offsetValue = getAttribute(target, 'offset') || `${floatingOffset}`;
  const trigger = getAttribute(target, 'trigger') || 'hover';
  const triggerout = getAttribute(target, 'triggerout') || 'hover';
  const canFlip = getAttribute(target, 'flip') === 'true';
