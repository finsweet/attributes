...
import { getAttribute, queryElement, type TooltipInstance } from '../utils';
 ...
 */
export const initTooltips = (targets: HTMLElement[]): TooltipInstance[] => {
  ...
  targets.forEach((target) => {
    ...
    instances.push(setupTooltip(target, tooltip, arrowElement));
  });
 ...
 */
export const initRctTooltips = (rctElements: HTMLDivElement[]) => {
  ...
  rctElements.forEach((element) => {
    ...
    tooltipTargets?.forEach((rctTarget) => {
      ...
      const rctInstance = setupTooltip(rctTarget, clonedTooltip, undefined);
